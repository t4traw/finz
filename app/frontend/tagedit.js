  ...
  tagger.fn = tagger.prototype = {
    ...
    _add_events: function () {
      ...
      this._new_input_tag.addEventListener('keydown', function (event) {
        if (event.keyCode === 13 || event.keyCode === 188 || (event.keyCode === 32 && !self._settings.allow_spaces)) {
          // enter || comma || space
...
import './stylesheet/tagger.scss'

tagger(document.querySelector('#tackle_tag_list'), {
  allow_spaces: false
})
